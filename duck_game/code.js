...
function refresh(){
	...
	for(let el of upper_backgrounds) {
		ctx.drawImage(el.pic, el.x, el.y, el.width, el.height);
	}
	...
	for(let el of enemies) {
		...
		if(el.name=='bird' && el.direction=='down') 
			...
			ctx.drawImage(el.pic, el.x, el.y, enemy_parameters[el.name].width, enemy_parameters[el.name].height);
	}
	...
	ctx.fillStyle = "pink";
	ctx.fillRect(screen_width*0.2, 100,screen_width*0.6, 5);
	ctx.stroke();
	
	ctx.fillStyle = "yellow";
	let img=new Image();
	img.src="character/duck_old.png";
	ctx.drawImage(img, screen_width*0.2, 100-duck.height, duck.width, duck.height);
	...
	if(hitbox_test_mod)
	{
		...
		for(enemy of enemies)
		{
			...
			{		
				ctx.rect(enemy.x+hitbox[0].x, enemy.y+hitbox[0].y,hitbox[1].x-hitbox[0].x, hitbox[1].y-hitbox[0].y);		
			}
		}
		for(hitbox of duck.hitbox)
		{		
			ctx.rect(duck.x+hitbox[0].x, duck.y+hitbox[0].y,hitbox[1].x-hitbox[0].x, hitbox[1].y-hitbox[0].y);		
		} 
		ctx.stroke();
	}
}
